...
import { BsCheck2All, BsPersonPlusFill } from "react-icons/bs";
import { getFecha } from "./RealizarVenta";
import { ListMetodoPago } from "../../../../Service/MetodoPagoService";
import { SearchClient } from "../ListarClientes/SearchClient";
...
export const InfoVenta = (props: any) => {
  const {
    data: data_metodoPago,
    isFetching,
    isLoading: loadingMetodoPago,
    isFetched,
  } = useQuery("metodo_pago", ListMetodoPago,
  ...
  const buscarCliente = () => {
    ...
    if (valor == "00000000") {
      ...
      mutateAsync(valor)
        .then((data: any) => {
          console.log("DATA DEL BUSCAR CLIENTE ...", data);
  ...
  const ModalConf = () => {
    ...
    const getProductosTabla = () => {
      ...
      props.vistaDescripcion.map((producto: any) => {
        productosTabla.push({
          DOC_ID: 0,
          PRO_ID: producto.PRO_FATHER_ID
            ? producto.PRO_FATHER_ID
            : producto.PRO_ID, //ID DEL PRODUCTO CON VALIDACION DE QUE SI ES UN HIJO O NO
          ...
          SDT_PRICE: (producto.inafecta == "1"
            ? producto.precio / 1.18
            : producto.precio
          ).toFixed(2), //PRECIO DEL PRODUCTO
          SDT_SUBTOTAL: (producto.inafecta == "1"
            ? producto.subtotal / 1.18
            : producto.subtotal
          ).toFixed(2), //CANTIDAD * PRECIO
          SDT_DISCOUNT: producto.descuento.toFixed(2), //DESCUENTO DEL PRODUCTO
          SDT_TOTAL: (producto.inafecta == "1"
            ? producto.total / 1.18
            : producto.total
          ).toFixed(2), //SUBTOTAL - DESCUENTO
    ...
    const saveDataVenta = () => {
      ...
      let dataVenta = {
        ...
        document: {
          ...
          POS_ID: props.formDetalle.POS_ID,
          //VALOR CAMBIA
          ...
          DOC_TAXED: (props.totalGravada + props.descuento) / 1.18, //TOTAL DEL SUBTOTAL SIN IGV CON DESCUENTO - TOTAL MONTO INAFECETA
          //SE DEJA EN 0.00
          ...
          //CAMBIA
          DOC_IGV: ((props.totalGravada + props.descuento) / 1.18) * 0.18, //IGV DEL DOC_TAXED - TOTAL IGV INAFECETA
          DOC_NETO: props.total.toFixed(2), //DOC_TAXED + DOC_IGV
          //CAMBIA - ESTADO DOCUMENTO
          DOC_STATUS: "CREADO", //HAY 2 ESTADO [CREADO,ACEPTADO ]
          DOC_DATE: getFecha(),
          DOC_METODO_PAGO: props.formDetalle.cuentaPago,
          DOC_USER_ID: admin.iu
        },
                ...
                <Box display={messageOfSended.status == 200 ? "block" : "none"}>
                  <Link href="/admin/ventas/listar-venta">
                    <Button
                      bg={"#2321fd"}
                      color={"white"}
                      _hover={{ bg: "rgb(237 242 247)", color: "#2321fd" }}
                      mr={3}
                    >
                      Ver Listado
                    </Button>
    ...
    <Box>
      ...
      >
        ...
        <Tabs isFitted variant='enclosed'  >
          <TabList  mb='1em'>
            <Tab _selected={{ color: 'white', bg: 'green.300' }  }>INFORMACIÓN DEL CLIENTE</Tab>
            <Tab _selected={{ color: 'white', bg: 'green.300' }  }>INFORMACIÓN DE LA VENTA</Tab>
          </TabList>
          <TabPanels>
            <TabPanel>

              <MyContain>
                <Box hidden={props.disabledClient}>
                  <Box
                    border="1px"
                    borderColor="gray.100"
                    w="100%"
                  ...
                  <Box w={'100%'} textAlign='center'>
                    <Button
                      ...
                      onClick={() => {
                        // console.log(props.formDetalle);
                        // console.log("ESTE ES EL LCT_ID QUE SE DEBE DE ENVIAR ... " + props.formDetalle.LCT_ID)
                        props.setDisabledClient(true)
                      }}>SELECCIONAR CLIENTE</Button>
                  </Box>
                </Box>

                <Box hidden={!props.disabledClient}>
                  <Box borderTopRadius={'10px'} backgroundColor={'#101938'} height='170px' > &nbsp;</Box>
                      <br />
                  <Box w={'100%'} display={'flex'} flexDirection='column' marginTop={'-110px'}>
                    <Box w={'100%'}  display={'flex'} justifyContent='center' alignItems={'center'}>
                      <img width={'40%'} style={{'borderRadius' : '100%'}} src="https://png.pngtree.com/png-vector/20191101/ourmid/pngtree-cartoon-color-simple-male-avatar-png-image_1934459.jpg" alt="" />
                    </Box>
                    <Box w={'100%'} textAlign='center'>
                      {/* <Text>CLIENTE: </Text> */}
                      <Text fontSize={'25px'} fontWeight='bold' textTransform={'uppercase'}>{props.formDetalle.razonSocial}</Text>

                      {/* <Text>ID CLIENTE: </Text> */}
                      <Text fontSize={'20px'} >{props.formDetalle.idCliente}</Text>

                      {/* <Text>DIRECCIÓN: </Text> */}
                      <Text  fontSize={'20px'} fontFamily='cursive' >{props.formDetalle.direccion}</Text>
                    </Box>
                  </Box>
                      <br />
                      <br />
                  <Box w={'100%'} textAlign='center'>
                    <Button
                      ...
                      _hover={{'backgroundColor' : 'white', 'color' : '#101938'}}
                      onClick={() => {
                        console.log(props.formDetalle);
                        console.log("ESTE ES EL LCT_ID QUE SE DEBE DE ENVIAR ... " + props.formDetalle.LCT_ID)
                        props.setDisabledClient(false)
                        props.setNextStep(false)
                        props.SetVistaDescripcion([])
